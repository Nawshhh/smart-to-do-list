    ...
    const tasks = [
        {
            task_name: "Test",
            task_priority: 1,
            task_completion_date: "January 24, 2024"
        }
    ]

    const priorityMap = { 1: "High", 2: "Medium", 3: "Low" };
    const priority = priorityMap[tasks[0].task_priority] ?? "";

    const bgMap = { 1: "bg-[#DE3D3D]", 2: "bg-[#3DDE3D]", 3: "bg-[#DEDE3D]" };
    const bgColor = bgMap[tasks[0].task_priority] ?? "";
        ...
        <div className='w-[467px] h-[82px] bg-[#212121] rounded-[10px] flex flex-row'>
            ...
            <div className='w-full h-full pl-[15px] pr-[13px] py-[15px]'>
                <div className='w-full h-full '>
                    <p className='font-helvetica font-medium text-white text-[20px]'>{tasks[0].task_name}</p>
                    <span className='flex flex-row'>
                        <p className='font-helvetica font-normal text-[#ADADAD] text-[14px] '>Complete by: {tasks[0].task_completion_date}</p>
